...
import { Milestone, Boxes, CheckCircle2, Clock, DollarSign, Package } from 'lucide-react'
...
export function ChannelAnalysis() {
  ...
  const channelCount = useMemo(() => distinctCount(rows, 'CHANNEL'), [rows])
  ...
  if (rows.length === 0) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-2 text-center text-muted-foreground">
        ...
        <p className="text-xs">Adjust the Global Filter Bar to see channel data.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5 pb-8">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <KpiCard label="Channels" value={formatNumber(channelCount)} icon={Milestone} />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <RankedBarList
          title="Top Districts"
          description="Within the selected channels — click to filter"
          buckets={byDistrict}
          limit={15}
        ...
        <DimensionBarChart
          ...
          description="Within the selected channels — click a bar to filter"
          buckets={byArea}
